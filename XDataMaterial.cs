...
namespace AVC
{
  ...
  XDataMaterial : XDataMan
  {
    ...
    /// <summary>
    /// Все материалы чертежа
    /// </summary>
    /// <param name="db"></param>
    /// <param name="tr"></param>
    /// <param name="excludeInherited">исключить byLayer, byBlock</param>
    /// <returns></returns>
    public static List<string> 
    GetMaterialList(HashSet<MatUseLike> useList, Database db, Transaction tr, bool excludeInherited = true)
    {
      List<string> ret = new();
      DBDictionary dic = tr.GetObject(db.MaterialDictionaryId, OpenMode.ForRead) as DBDictionary;
      if (dic is null) return ret;
      foreach (DBDictionaryEntry en in dic)
      {
        if (ColorExt.byLayer == en.Key || ColorExt.byBlock == en.Key) continue; // добавим их в потом, если надо
        Material mat = tr.GetObject(en.Value, OpenMode.ForRead) as Material;
        if (mat == null || mat.IsErased) continue;
        MatUseLike like = en.Key == MaterialExt.sGlobal ? MatUseLike.Unknown // на случай если пользователь поменял тип у Global - все равно считаем Unknown
          : new XDataMaterial(mat).Use;
        if (useList.Contains(like))
          ret.Add(en.Key);
      }

      if (!string.IsNullOrWhiteSpace(AvcSettings.TemplateFile))
      {
        using Database sourceDb = DatabaseExt.Open(AvcSettings.TemplateFile);
        if (sourceDb is null) return ret;
        if (sourceDb == db) return ret;   // вызов метода из самого файла шаблона
        using Transaction sourceTr = sourceDb.TransactionManager.StartTransaction();
        DBDictionary sourceMatDic = sourceTr.GetObject(sourceDb.MaterialDictionaryId, OpenMode.ForRead) as DBDictionary;
        if (sourceMatDic == null) return ret;
        foreach (DBDictionaryEntry en in sourceMatDic)
        {
          if (ColorExt.byLayer == en.Key 
            || ColorExt.byBlock == en.Key 
            || MaterialExt.sGlobal == en.Key
            || ret.Contains(en.Key)) continue;
          Material mat = sourceTr.GetObject(en.Value, OpenMode.ForRead) as Material;
          if (mat == null || mat.IsErased) continue;
          XDataMaterial xd = new (mat);
          if (useList.Contains(xd.Use))
            ret.Add(en.Key);
        }
        sourceTr.Commit();
      }
      ...
      if (!excludeInherited)
      {
        ret.Insert(0, ColorExt.LocalByBlock());
        ret.Insert(0, ColorExt.LocalByLayer());
      }
      return ret;
    }
